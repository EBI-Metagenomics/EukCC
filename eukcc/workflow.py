...
class eukcc():
    def __init__(self, options):
        ...
        # set name of run
        self.cfg['name'] = name = (os.path.splitext(
                                    os.path.basename(options.fasta))[0])
    ...
    def concatHMM(self):
        ...
        hmmdir = os.path.join(self.cfg['outdir'], "workfiles",
                              "hmmer", "estimations")
        file.isdir(hmmdir)
    ...
    def inferLineage(self, places):
        ...
        for p in places:
            ...
            if self.cfg['lineage'] == "limited":
                ...
                ranks2lineage = dict((rank, taxid) for (taxid, rank)
                                     in lineage2ranks.items())
                ranks = {'{}_id'.format(rank): ranks2lineage.get(rank, 'NA')
    ...
    def place(self, fasta, bedfile):
        ...
        # pplacer paths
        placerDir = os.path.join(self.cfg['outdir'],
                                 "workfiles", "pplacer")
        ...
        if not self.cfg['touch']:
            pplaceOutReduced = pp.reduceJplace(pplaceOut, pplaceOutReduced,
        ...
        if not self.cfg['touch']:
            ...
            for method in ['LCA', 'HPA']:
                self.placements[method] = t.getPlacement(method,
                                            sets,
                                            t2,
        ...
        with open(setinfo) as f:
            ...
            for line in f:
                ...
                for k, v in zip(cols, l):
                    n[k] = v
                if int(n["ngenomes"]) >= self.cfg['minGenomes'] and \
                   int(n['n']) >= self.cfg['minProfiles']:
